...
const Repo = ({ repo }) => {
	...
	const handleCloneClick = async (repo) => {
		try {
			...
			toast.success("Link da URL do Repositório copiado");
		} catch (error) {
			toast.error("A Cópia da URL falhou");
		}
	};
    ...
    rounded-full -start-3 ring-8 ring-white'
     ...
     text-gray-400'
			...
			<p className='mb-4 text-base font-normal text-gray-500'>
				{repo.description ? repo.description.slice(0, 500) : "Sem descrição fornecida"}
			</p>
